...
type RegionClient interface {
	SearchPoint(location regionclient.Coordinate, includeExtended bool) (regionclient.RegionList, error)
	SearchBox(boundingBox regionclient.BoundingBox, includeExtended bool) (regionclient.RegionList, error)
	SearchRadius(location regionclient.Coordinate, radiusKm float64, includeExtended bool) (regionclient.RegionList, error)
	FindCrossingLocations(fromRegion, toRegion string, fromLoc, toLoc regionclient.Coordinate, cfg regionclient.BorderCrossingConfig, limit int) ([]regionclient.BorderCrossing, error)
	FindRegionPath(fromRegion, toRegion string) ([]string, error)
}
...
func NewRegionHandler(client RegionClient) *RegionHandler {
	return &RegionHandler{client: client}
}
...
func (h *RegionHandler) FindCrossingLocations(w http.ResponseWriter, r *http.Request) {
	var req struct {
		...
		Config struct {
			Type          string   `json:"type"` // "simple" or "advanced"
...
func (h *RegionHandler) FindRegionPath(w http.ResponseWriter, r *http.Request) {
	...
	path, err := h.client.FindRegionPath(req.FromRegion, req.ToRegion)
	if err != nil {
		writeJSON(w, grpcStatus(err), errBody(err))
		return
	}
